...
{
    public class DialougeReader : PlayerRef
    {
        #region Singleton
        ...
        void Awake()
        {
            ...
            if (input == null)
            {
                if (!TargetCharacter.TryGetComponent(out input))
                {
                    Debug.Log("DialougeReader autoreference character failed, please set manually in inspector.");
                }
            }
            inputYes = input.actions["Yes"];
            inputNo = input.actions["No"];
            input1 = input.actions["1"];
            input2 = input.actions["2"];
            input3 = input.actions["3"];
            input4 = input.actions["4"];
        }
        #endregion

        public PlayerInput input;
        private InputAction inputYes;
        private InputAction inputNo;
