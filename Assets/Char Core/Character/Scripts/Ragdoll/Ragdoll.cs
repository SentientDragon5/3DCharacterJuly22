...
namespace Character
{
	...
	public sealed class Ragdoll :Moveable
	{
		...
		[SerializeField] private CapsuleCollider simpleCollider;


		public UnityEvent OnEnterRagdoll;

		/// <summary>
		/// Use this method to enter and exit ragdoll state when ready
		/// </summary>
		public void EnterRagdoll()
		{
			UnityEngine.Debug.Log("ragdolled");
			RagdollIn();
		...
		Animator _anim;
		Character6 _bzRagdollCharacter;
		...
		RagdollState _state
		{
			...
			set
            {
				...
				s = value;
				//disable/enable all colliders unless you are ragdolling.
				if (s == RagdollState.Animated)
				{
					...
					//foreach (Collider collider in colliders)
					//{
					//    if (collider.TryGetComponent<Animator>(out Animator anim))
					//    {
				...
				else
				{
					...
					//GetComponent<IController>().RagdollCollidersActive = true;
					//Collider[] colliders = GetComponentsInChildren<Collider>(true);
					//foreach (Collider collider in colliders)
					//{
					//    collider.enabled = true;
		...
		RagdollState s = RagdollState.Animated;
		...
		void Start()
		{
			...
			_bzRagdollCharacter = GetComponent<Character6>();
		...
		void FixedUpdate()
		{
			...
			*/
			...
			if(_state == RagdollState.Ragdolled && TryGetComponent(out Health h) && !h.IsDead && IsNearGround())
            {
				RagdollOut();
			}
		}
		float nearGroundDist = 1f;
		bool IsNearGround()
        {
			if(Physics.Raycast(_hipsTransform.position, Vector3.down, nearGroundDist))
            {
				return true;
            }
			return false;
		...
		void LateUpdate()
		{
			...
			{
				if (trComp.Transform != null)//Jan17 getting err. I added this to stop error. do not know why. same @line430ish
				{
        ...
        {
			foreach (var rigid in _rigids)
				rigid.RigidBody.velocity = velocity;
		}
		/// <summary>
		/// a small force to make it seem more realistic. push more on the top
		/// </summary>
		/// <param name="magnitude"></param>
		public void AddRandomVelocity(float magnitude)
        {
			foreach (var rigid in _rigids)
            {
				Vector3 rand = new Vector3(UnityEngine.Random.Range(-1, 1), UnityEngine.Random.Range(-1, 1), UnityEngine.Random.Range(-1, 1));
				rand.Normalize();
				rigid.RigidBody.velocity += rand * magnitude;
			}
		}
		public void AddForce(Vector3 position, Vector3 force, ForceMode mode)
        {
			foreach (var rigid in _rigids)
				rigid.RigidBody.AddForceAtPosition(force, position, mode);
		}
		public void AddExplosionForce(float force, Vector3 position, float radius, float upwards)
		{
			foreach (var rigid in _rigids)
				rigid.RigidBody.AddExplosionForce(force, position, radius, upwards);
		}
